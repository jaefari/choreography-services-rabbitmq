...
const main = async () => {
  ...
  // create exchange that this service will use and spread tasks
  await channel.assertExchange(apiExchangeName, 'direct', { durable: true });


  // /dateService/addDate/v1_2_3 /dateService/addDate/v1_2_0 /dateService/addDate/v1_0_0
  app.get('/:service/:intent/:intentVersion', async (req, res, next) => {
    try {
      const requestData = { data: 'data' };
      const { service, intent, intentVersion } = req.params;
      const taskId = uuid();

      logger.info({ message: 'get route called' });

      await publisher.publish({ service, intent, intentVersion, requestData, taskId });
